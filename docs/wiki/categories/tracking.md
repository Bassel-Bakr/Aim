...
below precise, reactive, and control.[^voltaic-s5]

[Aimlabs](../resources/trainers/aimlabs.md) frames the same split as "pure horizontal tracking and
smoothness" against "raw reactivity and target reading."[^aimlabs-apex]
...
settle without wobbling. A target that small leaves nowhere to hide a correction.[^voltaic-s5]
...
are rounded rather than sharp.[^voltaic-s5]
